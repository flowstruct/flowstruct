...
public class ProgramController {
  ...
  public ResponseEntity<ProgramDto> updateProgram(
      @PathVariable long programId, @Valid @RequestBody ProgramDetailsDto request) {
    return new ResponseEntity<>(
        programManagerService.editProgramDetails(programId, request), HttpStatus.OK);
  }

  @PostMapping
  public ResponseEntity<ProgramDto> createProgram(
      @Valid @RequestBody ProgramDetailsDto programDetails) {
    return new ResponseEntity<>(
        programManagerService.createProgram(programDetails), HttpStatus.CREATED);
  }

  @PutMapping("/{programId}/mark-outdated")
  public ResponseEntity<ProgramDto> markProgramOutdated(@PathVariable long programId) {
    return new ResponseEntity<>(
        programManagerService.markProgramOutdated(programId), HttpStatus.OK);
  }
