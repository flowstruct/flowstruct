...
public class CourseController {
  ...
  public ResponseEntity<CoursesPageDto> getPaginatedCourseList(
      ...
      @RequestParam(value = "status", defaultValue = "all", required = false)
          OutdatedFilter status) {
    return new ResponseEntity<>(
        courseService.getPaginatedCourseList(page, size, filter, status), HttpStatus.OK);
  }

  @GetMapping("/list")
  public ResponseEntity<List<CourseSummaryDto>> getCourseList(
      @RequestParam(value = "courses", defaultValue = "") List<Long> courseIds) {
    return new ResponseEntity<>(courseService.getCourseList(courseIds), HttpStatus.OK);
  }

  @GetMapping("/list/detailed")
  public ResponseEntity<List<CourseDto>> getCourseListDetailed(
      @RequestParam(value = "courses", defaultValue = "") List<Long> courseIds) {
    return new ResponseEntity<>(courseService.getCourseListDetailed(courseIds), HttpStatus.OK);
  }
  ...
  public ResponseEntity<CourseDto> editCourseDetails(
      @PathVariable long courseId, @Valid @RequestBody CourseDetailsDto courseDetails) {
    return new ResponseEntity<>(
        courseManagerService.editCourseDetails(courseId, courseDetails), HttpStatus.OK);
  }
  ...
  public ResponseEntity<CourseDto> createCourse(
    ...
    return new ResponseEntity<>(
        courseManagerService.createCourse(courseDetails), HttpStatus.CREATED);
  }

  @PutMapping("/{courseId}/mark-outdated")
  public ResponseEntity<CourseDto> markCourseOutdated(@PathVariable long courseId) {
    return new ResponseEntity<>(courseManagerService.markCourseOutdated(courseId), HttpStatus.OK);
  }

  @PutMapping("/{courseId}/mark-active")
  public ResponseEntity<CourseDto> markCourseActive(@PathVariable long courseId) {
    return new ResponseEntity<>(courseManagerService.markCourseActive(courseId), HttpStatus.OK);
  }
}
